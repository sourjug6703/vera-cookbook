...
function renderRecipe(recipe) {
  ...
  const sourceAction = recipe.sourcePreview?.pages?.length
    ? `<button class="source-open" type="button" aria-expanded="${state.sourceOpen}">${state.sourceOpen ? "Back to ingredients" : "View source"}</button>`
    : "";
  ...
  recipeReader.innerHTML = `
    <article class="reader-recipe">
      ...
      <div class="reader-recipe-body">
        ...
        ${history}
        <div class="reader-actions"><button class="reader-save" type="button" aria-pressed="${isSaved}" aria-label="${isSaved ? "Remove" : "Save"} ${escapeHtml(recipe.title)}">${isSaved ? "Saved recipe" : "Save recipe"}</button>${sourceAction}</div>
        ${state.sourceOpen ? sourceInline(recipe) : `<div class="reader-grid"><section><h2>Ingredients</h2><ul>${ingredientItems}</ul></section><section><h2>Preparation</h2><ol>${instructionItems}</ol>${notes}</section></div>`}
  ...
  readerSave.addEventListener("click", () => {
    ...
    readerSave.textContent = saved ? "Saved recipe" : "Save recipe";
  });
  recipeReader.querySelector(".source-open")?.addEventListener("click", () => state.sourceOpen ? closeSource() : openSource(recipe));
}

function sourceInline(recipe) {
  const pages = recipe.sourcePreview?.pages || [];
  return `
    <section class="source-inline" aria-labelledby="source-inline-title">
      <div class="source-inline-heading">
        <h2 id="source-inline-title" tabindex="-1">Source scan</h2>
        <p>This reading copy is derived from the retained scan for this source-checked recipe. Each page also links to its untouched original.</p>
      </div>
      <div class="source-pages">${pages.map((page) => `
        <figure class="source-figure">
          <picture>
            <source type="image/webp" srcset="${escapeHtml(sourceReaderWebp(page.image))}" />
            <img src="${escapeHtml(page.image)}" alt="Scanned cookbook page for ${escapeHtml(recipe.title)}" decoding="async" />
          </picture>
...
function openSource(recipe) {
  ...
  recipeReader.querySelector("#source-inline-title")?.focus({ preventScroll: true });
}
...
function closeSource() {
  ...
  render();
  window.scrollTo(scrollX, scrollY);
  recipeReader.querySelector(".source-open")?.focus({ preventScroll: true });
}
